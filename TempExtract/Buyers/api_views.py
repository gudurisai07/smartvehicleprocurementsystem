    ...
    try:
        if role == 'buyer':
            ...
            if user.status == 'Inactive':
                return Response({'error': 'Account is inactive. Please contact admin.'}, status=status.HTTP_403_FORBIDDEN)
...
def api_register(request):
    ...
    try:
        ...
        if role == 'buyer':
            ...
            user = userRegisteredTable(name=name, email=email, loginid=loginid, mobile=mobile, password=password, status='Active', location=location)
            try:
                user.full_clean()
                user.save()
            ...
            if sellerRegisteredTable.objects.filter(email=email).exists():
                return Response({'error': 'Email already registered.'}, status=status.HTTP_400_BAD_REQUEST)
            user = sellerRegisteredTable(name=name, email=email, loginid=loginid, mobile=mobile, password=password, status='Active')
            try:
                user.full_clean()
                user.save()
            except ValidationError as ve:
