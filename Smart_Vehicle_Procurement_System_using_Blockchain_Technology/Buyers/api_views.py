...
def api_register(request):
    ...
    try:
        ...
        if role == 'buyer':
            ...
            user = userRegisteredTable(name=name, email=email, loginid=loginid, mobile=mobile, password=password, status='waiting', location=location)
            try:
                user.full_clean()
                user.save()
            ...
            return Response({'error': 'Role "seller" is deprecated. Please register as "buyer" (unified user).'}, status=status.HTTP_400_BAD_REQUEST)
        return Response({'error': 'Invalid role.'}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
